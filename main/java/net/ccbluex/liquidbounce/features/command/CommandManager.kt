...
class CommandManager {
    ...
    fun executeCommands(input: String) {
        val chatbypass = LiquidBounce.moduleManager.chatBypass
        if (chatbypass.state && chatbypass.disablecommand.get()) {
            ClientUtils.displayChatMessage("§cTurn Fucking Disable Command off.")
            return
        }
