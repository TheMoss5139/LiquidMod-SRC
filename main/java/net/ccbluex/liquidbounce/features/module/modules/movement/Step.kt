...
class Step : Module() {
    ...
    private val modeValue = ListValue("Mode", arrayOf(
            "Vanilla", "Jump", "NCP", "MotionNCP", "OldNCP", "AAC", "LAAC", "AAC3.3.4", "Timer", "Spartan", "Rewinside"
    ), "NCP")
    ...
    private var isTimerChange = false
    ...
    fun onUpdate(event: UpdateEvent) {
        ...
        when {
            ...
            mode.equals("timer", true) -> {
                ...
                if (!mc.thePlayer.isCollidedHorizontally && mc.thePlayer.onGround
                    && !mc.gameSettings.keyBindJump.isKeyDown && isTimerChange) {
                    mc.timer.timerSpeed = 1F
                    isTimerChange = false
                }
            }
    ...
    fun onStep(event: StepEvent) {
        ...
        if (LiquidBounce.moduleManager.phase.state) {
            event.stepHeight = 0F
            return
        }

        // Some fly modes should disable step
        val fly = LiquidBounce.moduleManager.fly
        if (fly.state) {
        ...
        if (!mc.thePlayer.onGround || !timer.hasTimePassed(delayValue.get().toLong()) ||
            ...
            ||mode.equals("Timer", ignoreCase = true)) {
            mc.thePlayer.stepHeight = 0.5F
