...
public class NameProtect extends Module {
    ...
    public final BoolValue indianValue = new BoolValue("Indian", true);
    ...
    public void onText(final TextEvent event) {
        ...
        event.setText(StringUtils.replace(event.getText(), mc.thePlayer.getName(), ColorUtils.translateAlternateColorCodes(indianValue.get()? ForceUnicodeChat.convert(fakeNameValue.get()) : fakeNameValue.get()) + "§f"));

        if(allPlayersValue.get())
            for(final NetworkPlayerInfo playerInfo : mc.getNetHandler().getPlayerInfoMap())
                event.setText(StringUtils.replace(event.getText(), playerInfo.getGameProfile().getName(), indianValue.get()? ForceUnicodeChat.convert("Protected User") : "Protected User"));
    }
