...
class InventoryCleaner : Module() {
    ...
    private val items = arrayOf("None", "Ignore", "Sword", "Bow", "Pickaxe", "Axe", "Food", "Block", "Water", "Lava", "Gapple", "Pearl", "Rod")
    ...
    fun onUpdate(event: UpdateEvent) {
        ...
        val slientChestStealer = LiquidBounce.moduleManager.slientChestStealer
        if (slientChestStealer.state && slientChestStealer.listStack.isNotEmpty()) return
    ...
    fun isUseful(itemStack: ItemStack, slot: Int): Boolean {
        return try {
            ...
            if (item is ItemSword || item is ItemTool || item is ItemFishingRod) {
                ...
                for (i in 0..8) {
                    if (type(i).equals("sword", true) && item is ItemSword
                            ...
                            || type(i).equals("rod", true) && item is ItemFishingRod) {
                ...
                val damage = (itemStack.attributeModifiers["generic.attackDamage"].firstOrNull()?.amount
                        ?: 0.0) + 1.25 * if (item is ItemFishingRod) ItemUtils.getEnchantment(itemStack, Enchantment.knockback) else ItemUtils.getEnchantment(itemStack, Enchantment.sharpness)
            ...
            }else if (item is ItemBucket) {
                for (i in 0..8) {
                    if ((type(i).equals("Lava", true) && item.isFull == Blocks.flowing_lava)
                        || (type(i).equals("Water", true) && item.isFull == Blocks.flowing_water)) {
                        if (findBetterItem(i, mc.thePlayer.inventory.getStackInSlot(i)) == null) {
                            return true
                        }
                    }
                ...
                items(0, 45).none { (_, stack) -> itemStack != stack && stack.unlocalizedName == "item.compass" }
                    ...
                    item is ItemPotion || item is ItemEnderPearl || item is ItemEnchantedBook || itemStack.unlocalizedName == "item.stick" ||
        ...
        when (type.toLowerCase()) {
            ...
            "lava" -> {
                mc.thePlayer.inventory.mainInventory.forEachIndexed { index, stack ->
                    ...
                    if (item is ItemBucket && item.isFull == Blocks.flowing_lava && !type(index).equals("Lava", ignoreCase = true)) {
                        val replaceCurr = slotStack == null || slotStack.item !is ItemBucket || (slotStack.item as ItemBucket).isFull != Blocks.flowing_lava
            ...
            "rod" -> {
                var bestRod = if (slotStack?.item is ItemFishingRod) targetSlot else -1
                var bestKnockPower = if (bestRod != -1)
                    ItemUtils.getEnchantment(slotStack, Enchantment.knockback)
                else
                    0

                mc.thePlayer.inventory.mainInventory.forEachIndexed { index, itemStack ->
                    if (itemStack?.item is ItemFishingRod && !type(index).equals(type, ignoreCase = true)) {
                        if (bestRod == -1) {
                            bestRod = index
                        } else {
                            val power = ItemUtils.getEnchantment(itemStack, Enchantment.knockback)

                            if (ItemUtils.getEnchantment(itemStack, Enchantment.knockback) > bestKnockPower) {
                                bestRod = index
                                bestKnockPower = power
                            }
                        }
                    }
                }

                return if (bestRod != -1) bestRod else null
            }
        }
