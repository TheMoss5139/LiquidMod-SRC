...
class AutoWeapon : Module() {
    ...
    private val fastchange = BoolValue("FastChange", false)
    ...
    fun onPacket(event: PacketEvent) {
        if (event.packet is C02PacketUseEntity && event.packet.action == C02PacketUseEntity.Action.ATTACK
            ...
            if (fastchange.get()) mc.netHandler.addToSendQueue(C09PacketHeldItemChange(mc.thePlayer.inventory.currentItem))
            event.cancelEvent()
        }
    }
