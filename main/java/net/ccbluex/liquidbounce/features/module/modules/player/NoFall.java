...
public class NoFall extends Module {
    public final ListValue modeValue = new ListValue("Mode", new String[]{"SpoofGround", "NoGround", "Packet", "MLG", "AAC", "LAAC", "AAC3.3.11", "AAC3.3.15", "AAC4.3.12", "Spartan", "CubeCraft", "Hypixel"}, "SpoofGround");
    private final FloatValue minFallDistance = new FloatValue("MinMLGHeight", 5F, 2F, 50F);
    private IntegerValue tickpackets = new IntegerValue("Ticks", 20, 1, 40);
    ...
    private Vec3 startNoFall = null;
    private double[] motion = null;
    ...
    public void onUpdate(UpdateEvent event) {
        ...
        if (!getState() || LiquidBounce.moduleManager.getFreeCam().getState())
            return;
        ...
        switch (modeValue.get().toLowerCase()) {
            ...
            case "spartan":
                ...
                break;
            case "aac4.3.12":
                if (mc.thePlayer.fallDistance > 2 && !isVoid()) {
                    disablelogger = true;
                    startNoFall = mc.thePlayer.getPositionVector();
                    motion = new double[] {mc.thePlayer.motionX, mc.thePlayer.motionY, mc.thePlayer.motionZ};
                    rotation = new Rotation(mc.thePlayer.rotationYaw, mc.thePlayer.rotationPitch);
                }
                if (mc.thePlayer.onGround && disablelogger) {
                    disablelogger = false;
    ...
    public void onPacket(final PacketEvent event) {
        ...
        if (mode.equalsIgnoreCase("AAC4.3.12")) {
            if (packet instanceof S08PacketPlayerPosLook) {
                packets.clear();
                disablelogger = false;
                if (startNoFall != null && motion != null && rotation != null) {
                    mc.thePlayer.setPositionAndRotation(startNoFall.xCoord, startNoFall.yCoord, startNoFall.zCoord, rotation.getYaw(), rotation.getPitch());
                    mc.thePlayer.motionX = motion[0];
                    mc.thePlayer.motionY = motion[1];
                    mc.thePlayer.motionZ = motion[2];
                    startNoFall = null;
                    motion = null;
                    rotation = null;
                }
            ...
            if (packet instanceof S12PacketEntityVelocity) {
                if (((S12PacketEntityVelocity) packet).motionY <= 0)
                    return;
                packets.clear();
                disablelogger = false;
                if (startNoFall != null && motion != null && rotation != null) {
            ...
            if (packet instanceof C03PacketPlayer) {
                ...
                if (disablelogger) {
                    if (mc.thePlayer.fallDistance > 3 && GetPacketType(playerPacket)) {
                        mc.thePlayer.fallDistance = 0;
                        playerPacket.onGround = true;
                    }
                    packets.add(playerPacket);
                    event.cancelEvent();
                }
            }
            return;
    ...
    public boolean GetPacketType(Packet packet) {
    ...
    public boolean isVoid() {
        return mc.theWorld.getCollidingBoundingBoxes(mc.thePlayer, new AxisAlignedBB(mc.thePlayer.posX, mc.thePlayer.posY, mc.thePlayer.posZ, mc.thePlayer.posX, 0, mc.thePlayer.posZ)).isEmpty();
    }
