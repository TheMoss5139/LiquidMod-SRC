...
class Criticals : Module() {
    ...
    val modeValue = ListValue("Mode", arrayOf("Packet", "NcpPacket", "NoGround", "Hop", "TPHop", "Jump", "LowJump", "AAC4.3.11"), "packet")
    ...
    fun onAttack(event: AttackEvent) {
        if (event.targetEntity is EntityLivingBase) {
            ...
            if (!mc.thePlayer.onGround || mc.thePlayer.isOnLadder || mc.thePlayer.isInWeb || mc.thePlayer.isInWater ||
                    ...
                    LiquidBounce.moduleManager.fly.state || !msTimer.hasTimePassed(delayValue.get().toLong()))
                return
            ...
            when (modeValue.get().toLowerCase()) {
                ...
                "aac4.3.11" -> {
                    if (mc.thePlayer.motionX != 0.0 || mc.thePlayer.motionZ != 0.0 || mc.thePlayer.motionY != 0.0) return
                    ...
                    if(killAura.state && killAura.target != null) {
                        ...
                        val pitch = RotationUtils.targetRotation.pitch
                        mc.netHandler.addToSendQueue(C03PacketPlayer.C06PacketPlayerPosLook(x, y + 1.949e-13, z, yaw, pitch, false))
                        mc.netHandler.addToSendQueue(C03PacketPlayer.C06PacketPlayerPosLook(x, y + 1.153e-13, z, yaw, pitch, false))
                    }else{
                        mc.netHandler.addToSendQueue(C04PacketPlayerPosition(x, y + 1.949e-13, z, false))
                        mc.netHandler.addToSendQueue(C04PacketPlayerPosition(x, y + 1.153e-13, z, false))
                    }
                }
