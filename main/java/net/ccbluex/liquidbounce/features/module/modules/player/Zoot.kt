...
class Zoot : Module() {
    ...
    fun onUpdate(event: UpdateEvent) {
        ...
        if (badEffectsValue.get() && hasBadEffect()) {
        ...
        if (fireValue.get() && !mc.thePlayer.capabilities.isCreativeMode && mc.thePlayer.isBurning && !mc.thePlayer.isInLava && !mc.thePlayer.isPotionActive(Potion.fireResistance)) {
            repeat(9) {
                mc.netHandler.addToSendQueue(C03PacketPlayer(mc.thePlayer.onGround))
            }
        }
