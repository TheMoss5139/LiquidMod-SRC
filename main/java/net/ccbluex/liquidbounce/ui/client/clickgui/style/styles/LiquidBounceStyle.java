...
public class LiquidBounceStyle extends Style {
    ...
    public void drawPanel(int mouseX, int mouseY, Panel panel) {
        ...
        if(panel.getScrollbar() && panel.getFade() > 0) {
            ...
            RenderUtils.drawRect(panel.getX() - 2, panel.getY() + 30 + (panel.getFade() - 24F) / (panel.getElements().size() - LiquidBounce.moduleManager.getClickGUI().maxElementsValue.get()) * panel.getDragged() - 10.0f, panel.getX(), panel.getY() + 40 + (panel.getFade() - 24.0f) / (panel.getElements().size() - LiquidBounce.moduleManager.getClickGUI().maxElementsValue.get()) * panel.getDragged(), Integer.MIN_VALUE);
        }
    }
    ...
    public void drawModuleElement(int mouseX, int mouseY, ModuleElement moduleElement) {
        ...
        if(!moduleValues.isEmpty()) {
            ...
            if(moduleElement.isShowSettings()) {
                int yPos = moduleElement.getY() + 4 + addedY;
                int startY = yPos;
                int startX = moduleElement.getX() + moduleElement.getWidth() + 4;
                ...
                if (!isChanged) {
                    if (mouseX >= startX && mouseX <= moduleElement.getX() + moduleElement.getWidth() + moduleElement.getSettingsWidth() && mouseY >= startY && mouseY <= yPos) {
