...
public class RotationUtils {
    ...
    public static float[] getPredictedRotations(EntityLivingBase ent) {
        ...
        double y = ent.posY + ent.getEyeHeight();
        return getRotationFromPosition(x, z, y);
    }
    ...
    public static float getPitchChange(float pitch, Entity entity, double posY) {
    	...
    	double deltaY = (posY + entity.getEyeHeight() / 2) - (mc.thePlayer.posY + mc.thePlayer.getEyeHeight());
    	double distanceXZ = MathHelper.sqrt_double(deltaX * deltaX + deltaZ * deltaZ);
    	double pitchToEntity = -Math.toDegrees(Math.atan(deltaY / distanceXZ));
    	return -MathHelper.wrapAngleTo180_float(pitch - (float) pitchToEntity);
    }
    
    public static float getReturnYawChange(float yaw) {
        return MathHelper.wrapAngleTo180_float(-(yaw - mc.thePlayer.rotationYaw));
    }

    public static float getReturnPitchChange(float pitch) {
        return -MathHelper.wrapAngleTo180_float(pitch - mc.thePlayer.rotationPitch);
    }
    ...
    public static float getDistanceBetweenAngles(float angle1, float angle2) {
        ...
        return angle;
//    	return ((((angle1 - angle2) % 360F) + 540F) % 360F) - 180F;
    }

    public static float[] limitAngleChange(float[] currentRotation, float[] targetRotation, final float turnSpeed) {
        final float yawDifference = getDistanceBetweenAngles(currentRotation[0], targetRotation[0]);
        final float pitchDifference = getDistanceBetweenAngles(targetRotation[1], currentRotation[1]);
