...
public class RenderingUtil extends MinecraftInstance {
...
//        Tessellator.getInstance().draw();
    ...
    public static void drawLine3D(float x, float y, float z, float x1, float y1, float z1, int color) {
        ...
        glVertex3d(x - renderPosX, y - renderPosY, z - renderPosZ);
        glVertex3d(x1 - renderPosX, y1 - renderPosY, z1 - renderPosZ);
        GL11.glEnd();
        post3D();
    }
    
    public static void draw3DLine(float x, float y, float z, float x1, float y1, float z1, int color) {
        pre3D();
        GL11.glLoadIdentity();
        mc.entityRenderer.setupCameraTransform(mc.timer.renderPartialTicks, 2);
        float var11 = (color >> 24 & 0xFF) / 255.0F;
