...
public final class EntityUtils extends MinecraftInstance {
            ...
            if(targetInvisible || !entity.isInvisible()) {
                if(targetPlayer && entity instanceof EntityPlayer) {
                    ...
                    if(canAttackCheck) {
                        ...
                        if (isFriend(entityPlayer) && !LiquidBounce.moduleManager.getNoFriends().getState())
                            return false;

                        if(entityPlayer.isSpectator())
                            return false;

                        final Teams teams = LiquidBounce.moduleManager.getTeams();
    ...
    public static boolean isFriend(final Entity entity) {
        Module module = LiquidBounce.moduleManager.getNoFriends();
        return entity instanceof EntityPlayer && entity.getName() != null &&
                LiquidBounce.fileManager.friendsConfig.isFriend(ColorUtils.stripColor(entity.getName())) && !module.getState();
    }
