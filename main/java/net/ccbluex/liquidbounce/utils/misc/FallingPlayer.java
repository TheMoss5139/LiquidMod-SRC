...
public class FallingPlayer extends MinecraftInstance {
    ...
    public ArrayList<C03PacketPlayer.C04PacketPlayerPosition> calculatePacket(int ticks) {
        CollisionResult result = findCollision(ticks);
        if (result == null)
            return null;
        ArrayList<C03PacketPlayer.C04PacketPlayerPosition> packets = new ArrayList<>();
        double lastY = y;
        for (int i = 0; i - 1 < result.getTick(); i++) {
            calculateForTick();
            ...
            packets.add(new C03PacketPlayer.C04PacketPlayerPosition(x,y,z, onGround));
        }
        return packets;
    }
