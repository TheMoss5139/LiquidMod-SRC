...
import net.ccbluex.liquidbounce.features.module.modules.render.DebugESP;
import net.ccbluex.liquidbounce.features.module.modules.world.Scaffold;
import net.ccbluex.liquidbounce.utils.misc.RandomUtils;
import net.ccbluex.liquidbounce.utils.renon.Utils;
...
public final class RotationUtils extends MinecraftInstance implements Listenable {
    ...
    public static void faceBow(final Entity target, final boolean silent, final boolean predict, final float predictSize) {
        ...
        float velocity = LiquidBounce.moduleManager.getFastBow().getState() ? 1F : player.getItemInUseDuration() / 20F;
    ...
    public static void faceBow(final Entity target,final Vec3 vec3, final boolean silent) {
        final EntityPlayerSP player = mc.thePlayer;

        final double posX = vec3.xCoord - player.posX;
        final double posY = vec3.yCoord + target.getEyeHeight() - 0.15 - player.getEntityBoundingBox().minY - player.getEyeHeight();
        final double posZ = vec3.zCoord - player.posZ;
        final double posSqrt = Math.sqrt(posX * posX + posZ * posZ);
     ...
     * @return rotation
     */
    public static Rotation toRotation(Vec3 vec, final boolean predict) {
    ...
    public static VecRotation searchCenter(final AxisAlignedBB bb, final boolean outborder, final boolean random,
        ...
        for(double xSearch = 0.45D; xSearch < 0.55D; xSearch += 0.05D) {
            for (double ySearch = 0.35D; ySearch < 0.75D; ySearch += 0.05D) {
                for (double zSearch = 0.45D; zSearch < 0.55D; zSearch += 0.05D) {
    ...
    public static VecRotation searchCenter(final AxisAlignedBB bb, final boolean random,
                                           final boolean predict, final boolean throughWalls, final float distance, Vec3 randomValue) {
        final Vec3 eyes = mc.thePlayer.getPositionEyes(1F);

        VecRotation vecRotation = null;
        Vec3 vec;
        if (!random) {
            vec = new Vec3(((bb.minX + bb.maxX) / 2),
                    ((bb.minY + bb.maxY) / 2), ((bb.minZ + bb.maxZ) / 2));
            final Rotation rotation = toRotation(vec, predict);

            if(throughWalls || isVisible(vec)) {
                vecRotation = new VecRotation(vec, rotation);
            }
        }else {
            vec = new Vec3((((bb.minX + bb.maxX) / 2) + randomValue.xCoord),
                    (((bb.minY + bb.maxY) / 2) + randomValue.yCoord), (((bb.minZ + bb.maxZ) / 2) + randomValue.zCoord));
            final Rotation rotation = toRotation(vec, predict);
        ...
        for(double xSearch = bb.minX; xSearch <= bb.maxX; xSearch += (bb.maxX - bb.minX) / 5) {
            for (double ySearch = bb.minY; ySearch <= bb.maxY; ySearch += (bb.maxY - bb.minY) / 5) {
                for (double zSearch = bb.minZ; zSearch <= bb.maxZ; zSearch += (bb.maxZ - bb.minZ) / 5) {
                    vec = new Vec3(xSearch,
                            ySearch, zSearch);
                    final Rotation rotation = toRotation(vec, predict);
                    final double vecDist = eyes.distanceTo(vec);

                    if (vecDist > distance)
                        continue;

                    if(throughWalls || isVisible(vec)) {
                        final VecRotation currentVec = new VecRotation(vec, rotation);
    ...
    public static double getRotationDifference2(final Rotation rotation) {
        return serverRotation == null ? 0D : getRotationDifference2(rotation, serverRotation);
    }
    ...
    public static double getRotationDifference2(final Rotation a, final Rotation b) {
        return Math.hypot(getAngleDifference(a.getYaw(), b.getYaw()), 0);
    }
    ...
    @NotNull
    public static Rotation limitAngleChange2(final Rotation currentRotation, final Rotation targetRotation, final float turnSpeed, final float multiplier) {
        final float yawDifference = getAngleDifference(targetRotation.getYaw(), currentRotation.getYaw());
        final float pitchDifference = getAngleDifference(targetRotation.getPitch(), currentRotation.getPitch());

        return new Rotation(
                currentRotation.getYaw() + (yawDifference > turnSpeed ? turnSpeed : Math.max(yawDifference, -turnSpeed)) * multiplier,
                currentRotation.getPitch() + (pitchDifference > turnSpeed ? turnSpeed : Math.max(pitchDifference, -turnSpeed) * multiplier
                ));
    }

    @NotNull
    public static Rotation limitAngleChange3(final Rotation currentRotation, final Rotation targetRotation, float multiplier) {
        ...
        if (yawDifference != 0) {
            if (yawDifference > 0) {
                randomYaw = RandomUtils.nextFloat(0, yawDifference * multiplier);
            } else {
                randomYaw = RandomUtils.nextFloat(yawDifference * multiplier, 0);
            }
        }
        if (pitchDifference != 0) {
            if (pitchDifference > 0) {
                randomPitch = RandomUtils.nextFloat(0, pitchDifference * multiplier);
            } else {
                randomPitch = RandomUtils.nextFloat(pitchDifference * multiplier, 0);
            }
        }
        return new Rotation(
                currentRotation.getYaw() + randomYaw,
                currentRotation.getPitch() + randomPitch);
    }
    ...
    public void onPacket(final PacketEvent event) {
        ...
        if(packet instanceof C03PacketPlayer) {
            ...
            if(targetRotation != null && !keepCurrentRotation && (targetRotation.getYaw() != serverRotation.getYaw() || targetRotation.getPitch() != serverRotation.getPitch())) {
                packetPlayer.yaw = getNewAngle(targetRotation.getYaw());
            ...
            if (packetPlayer.rotating) {
                DebugESP debugESP = LiquidBounce.moduleManager.getDebugESP();
                if (debugESP.getState() && targetRotation != null) {
                    if (debugESP.getScaffoldValue().get()) {
                        Scaffold scaffold = LiquidBounce.moduleManager.getScaffold();
                        if (scaffold.getState()) {
                            debugESP.getRotationArray().add(targetRotation);
                        }
                    }
                    if (debugESP.getKillAuraValue().get()){
                        KillAura aura = LiquidBounce.moduleManager.getKillAura();
                        if (aura.getState()) {
                            debugESP.getRotationArray().add(targetRotation);
                        }
                    }
