...
public final class InventoryUtils extends MinecraftInstance implements Listenable {
    ...
    public static int findItem(final int startSlot, final int endSlot, final Item item, int[] ignoreslot) {
        for(int i = startSlot; i < endSlot; i++) {
            for (int slot : ignoreslot) {
                if (i == slot) {
                    continue;
                }
            }
    ...
    public static int findItem(final int startSlot, final int endSlot, final Item item, final int ignoreslot) {
        for(int i = startSlot; i < endSlot; i++) {
//            if (ignoreslot != null) {
//                for (int ignore = 0; ignore < ignoreslot.length; ignore++) {
//                }
//            }
            if (i == ignoreslot) {
                continue;
    ...
    public static boolean hasSpaceHotbar(int ignoreSlot) {
        for(int i = 36; i < 45; i++) {
//            if (ignoreSlot != null) {
//                for (int j = 0; j < ignoreSlot.length; j++) {
//                }
//            }
            if (i == ignoreSlot) {
                continue;
    ...
    public static int findSpaceHotbar(int ignoreSlot) {
        for(int i = 0; i < 9; i++) {
//            for (int j = 0; j < ignoreSlot.length; j++) {
//            }
            if (i + 36 == ignoreSlot) {
            ...
            final ItemStack itemStack = mc.thePlayer.inventoryContainer.getSlot(i + 36).getStack();
